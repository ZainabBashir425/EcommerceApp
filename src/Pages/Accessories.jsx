...
export default function Accessories() {
  ...
  document.title = "Fashion Aura | Accessories";

  const accessoriesCategory = allProducts.find(
    (category) => category.category === "Accessories"
  );

  useEffect(() => {
    if (accessoriesCategory && accessoriesCategory.products.length > 0) {
      const { min, max } = calculateMinMaxPrice(accessoriesCategory.products);
      setMinPrice(min);
      setMaxPrice(max);
  ...
  }, [accessoriesCategory, priceRange, selectedSortOption, selectedSubcategories]);
