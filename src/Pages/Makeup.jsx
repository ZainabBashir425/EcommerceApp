...
export default function Makeup() {
  ...
  document.title = "Fashion Aura | Makeup";

  const makeupCategory = allProducts.find(
    (category) => category.category === "Makeup"
  );

  useEffect(() => {
    if (makeupCategory && makeupCategory.products.length > 0) {
      const { min, max } = calculateMinMaxPrice(makeupCategory.products);
      setMinPrice(min);
      setMaxPrice(max);
  ...
  }, [makeupCategory, priceRange, selectedSortOption, selectedSubcategories]);
                ...
                {isPOpen && (
                  <div className="filter-content">
                    <ul className="checkbox-group">
                      {makeupCategory.subcategories.map((subcategory) => (
                        <li key={subcategory}>
                          <input
                            type="checkbox"
                            id={subcategory}
